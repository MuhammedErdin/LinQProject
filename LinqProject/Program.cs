...
namespace LinQProject
{
    class Program
    {
        ...
        {
            var result = products.Find(p => p.ProductId == 3);      //Find aradığımız kritere uygun nesneyi verir
            Console.WriteLine(result);
        }

        private static void AnyTest(List<Product> products)
        {
            var result = products.Any(p => p.ProductName == "Acer Laptop");     //Any ile bir listenin içerisinde bir eleman var mı yok mu onu aramamıza yarar
            Console.WriteLine(result);
        }
